...
export function generateSurgeModule(config: ModuleConfig, options: GenerateOptions = {} as GenerateOptions): string {
  const lines: string[] = []
  ...
  if (config.dns && Object.keys(config.dns).length > 0) {
    lines.push("[DNS]")
    for (const [domain, server] of Object.entries(config.dns)) {
